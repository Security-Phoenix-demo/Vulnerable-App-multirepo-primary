...
# Flags:
#   --apply               Write files
#   --also-companion-app  Also generate callable + unreachable exploit-hunt companion app files

MODE="dry-run"
ALSO_COMPANION_APP="false"
for arg in "$@"; do
  case "$arg" in
    --apply)
      MODE="apply"
...
if [[ "$ALSO_COMPANION_APP" == "true" ]]; then
  if [[ "$MODE" == "apply" ]]; then
    "$ROOT_DIR/scripts/generate-exploit-hunt-fixtures.sh" --apply
  else
    "$ROOT_DIR/scripts/generate-exploit-hunt-fixtures.sh"
  fi
fi
